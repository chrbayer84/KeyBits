...
public class KeyBits {
	...
	public static void main(String[] args) throws Exception{
		...
		Option imprt					= OptionBuilder.withArgName( "string" )   .hasArg().withDescription( "import public key from blockchain").create( "i" );
		
		Option blockchain_address		= OptionBuilder.withArgName( "string" )   .hasArg().withDescription( "bitcoin address" ).create( "a" );
		Option create_wallet			= OptionBuilder.withArgName( "file name" ).hasArg().withDescription( "create wallet" ).create( "c" );
		Option update_wallet			= OptionBuilder.withArgName( "file name" ).hasArg().withDescription( "update wallet" ).create( "u" );
		Option balance_wallet			= OptionBuilder.withArgName( "file name" ).hasArg().withDescription( "return balance of wallet" ).create( "b" );
		Option show_wallet				= OptionBuilder.withArgName( "file name" ).hasArg().withDescription( "show content of wallet" ).create( "w" );
		Option send_coins				= OptionBuilder.withArgName( "file name" ).hasArg().withDescription( "send satoshis" ).create( "s" );
		Option monitor_pending			= OptionBuilder.withArgName( "file name" ).hasArg().withDescription( "monitor pending transactions of wallet" ).create( "p" );
		Option monitor_depth			= OptionBuilder.withArgName( "file name" ).hasArg().withDescription( "monitor transaction depths of wallet" ) .create( "d" );
		Option number					= OptionBuilder.withArgName( "integer" )  .hasArg().withDescription( "in combination with -c, -d, -r or -s" ).create( "n" );
		...
		if (cmd.hasOption("c")){
			...
			String passphrase = HelpfullStuff.  insertPassphrase("enter password for " + wallet_file_name);
			if (!new File(wallet_file_name).exists()){
				String passphrase_= HelpfullStuff.reInsertPassphrase("enter password for " + wallet_file_name);
		...
		if (cmd.hasOption("b")){
			...
			System.out.println(MyWallet.getBalanceOfWallet(wallet_file_name, wallet_file_name + ".chain").longValue());
			System.exit(0);
		}
		...
		if (cmd.hasOption("d")){
			...
			MyWallet.monitorTransactionDepth(wallet_file_name, wallet_file_name + ".chain", checkpoints_file_name, depth);
			System.exit(0);
		}
		...
		if (cmd.hasOption("s") && cmd.hasOption("a") && cmd.hasOption("n")){
			...
			String wallet_passphrase = HelpfullStuff.insertPassphrase("enter password for " + wallet_file_name);
			MyWallet.sendCoins(wallet_file_name, wallet_file_name + ".chain", checkpoints_file_name, address, new BigInteger(amount + ""), wallet_passphrase);
...
//			System.out.println(gpg.showPublicKey(gpg.getDecoded(encoding.decodePublicKey(addresses))));
			...
			String public_key_wallet_passphrase = HelpfullStuff.insertPassphrase("enter password for " + public_key_wallet_file_name);
			if (!new File(public_key_wallet_file_name).exists()){
				String public_key_wallet_passphrase_= HelpfullStuff.reInsertPassphrase("enter password for " + public_key_wallet_file_name);
			...
			MyWallet.createWallet(public_key_wallet_file_name, public_key_chain_file_name, 1, public_key_wallet_passphrase);
			MyWallet.updateWallet(public_key_wallet_file_name, public_key_chain_file_name, checkpoints_file_name);
			String public_key_address = MyWallet.getAddress(public_key_wallet_file_name, public_key_chain_file_name, 0);
			...
			KeyBits.encodeAddresses(public_key_wallet_file_name, public_key_chain_file_name, checkpoints_file_name, addresses, 2*SendRequest.DEFAULT_FEE_PER_KB.intValue(), depth, public_key_wallet_passphrase);
		}
	...
	public static void encodeAddresses(String wallet_file_name, String chain_file_name, String checkpoints_file_name, String[] addresses, int amount, int depth, String passphrase) throws Exception{
		...
		System.out.println("available funds  : " + available + " satoshis (" + available.longValue()/satoshis_of_bitcoin + " XBT)");
		System.out.println("needed fees      : " + fee + " satoshis (" + fee/satoshis_of_bitcoin + " XBT)");
		System.out.println("needed funds     : " + needed + " satoshis (" + needed.longValue()/satoshis_of_bitcoin + " XBT)");
		...
		if (available.longValue() >= needed.longValue()){
			createAddresses(wallet_file_name, chain_file_name, checkpoints_file_name, addresses, new BigInteger(fee + ""), new BigInteger(amount_per_address + ""), depth, passphrase);
		} else {
			...
			System.out.println();
			System.out.println("please send " + to_send.longValue() + " satoshis (" + to_send.longValue()/satoshis_of_bitcoin + " XBT) to address " + address);
			
			System.exit(0);
		}		
	}

	public static void createAddresses(String wallet_file_name, String chain_file_name, String checkpoints_file_name, String[] addresses, BigInteger transaction_cost, BigInteger amount_per_address, int depth, String passphrase) throws Exception{
		if (HelpfullStuff.yesNo("create addresses?") == 1){
			try {
				System.out.println("creating addresses of keys ... ");
				MyWallet.createKeys(wallet_file_name, chain_file_name, checkpoints_file_name, addresses, transaction_cost, amount_per_address, passphrase);

...
public class Encoding
{
    ...
    {
        byte[] xork = HelpfulStuff.getXorKey( 20 );
        byte[] encrypted_public_key = HelpfulStuff.xor( public_key, xork );
        byte[] xork_and_encrypted_public_key = HelpfulStuff.concat( xork, encrypted_public_key );

        // add size
        byte[] bsize = HelpfulStuff.intToByteArray( xork_and_encrypted_public_key.length );
        byte[] size_and_xork_and_encrypted_public_key = HelpfulStuff.concat( bsize, xork_and_encrypted_public_key );
     ...
     * @throws Exception
     */
    public byte[] decodePublicKey( String[] addresses )
        throws Exception
    {
        ...
        for ( int i = 0; i < addresses.length; i++ )
            keyparts[i] = HelpfulStuff.subArray( Base58.decode( addresses[i] ), 1, 20 );
        ...
        byte[] xork_and_encrypted_public_key =
            HelpfulStuff.subArray( size_and_xork_and_encrypted_public_key, 20,
                size_and_xork_and_encrypted_public_key.length - 20 );
        byte[] xork = HelpfulStuff.subArray( size_and_xork_and_encrypted_public_key, 0, 20 );
        byte[] public_key = HelpfulStuff.xor( xork_and_encrypted_public_key, xork );
    ...
    public byte[] decode( byte[][] data )
        throws Exception
    {
        int size = HelpfulStuff.byteArrayToInt( new byte[] { data[0][0], data[0][1], data[0][2], data[0][3] } );
     ...
     * @return
     */
    private byte[] decode_( byte[][] data, int size )
        throws Exception
    {
        if ( size > 16384 )
